...
1. 线上文档, 放在wiki
2. Proj1, 单体python后端应用(待定), 需要有前端, Proj2, Go微服务架构的电商系统(去年青训营), 都是小组作业(git), 需要简单前端界面. (Proj1前端组摇人或者cursor)
3. 暑假把第一部分教程完成, 完成一部分Proj2内容. 一个学期学习一个部分. 第一学期需要考虑达到CS61A的水准, 第二学期Proj2耗时较长, 可能需要两三个月