...
如果这种边界投影能够保持原图中的推导方案和成本语义，知识图就可以在不同抽象尺度之间切换：封装一个子图时，系统可以根据原边界计算它与外部概念的关系；这个新父概念还可以继续和其他概念组合，递归形成更大的 DAG。图中只展示了边的直观投影，什么情况下这种封装不会改变 AND/OR 推导语义，仍然是模型必须解决的问题。
...
例如，假设被封装子图中的目标必须由 $B$ 和 $C$ 共同推出，而外部概念 $X$ 只能抵达 $B$，$Z$ 只能抵达 $C$。如果折叠后只是机械地生成 $X \to P$ 和 $Z \to P$ 两条普通入边，并分别把它们当成单边方案，原来的 $X \land Z$ 就会被错误地改写成 $X \lor Z$。同理，一条从内部边缘节点出发的边，也未必意味着只要“理解了父概念”就一定拥有使用这条出边所需的具体内部结论。
...
如果仍然要封装一般子 DAG，父概念就必须拥有明确的接口，而不能只继承一组扁平的边。这个接口至少需要保留：

1. 哪些内部节点是入口，外部概念能够为它们提供什么前提；
2. 哪些入口边必须共同满足，哪些入口组合互为替代方案；
3. 父概念向外部导出的究竟是哪一个内部结论；
4. 使用某条出边时，要求内部完成哪一张有效推导子图；
5. 折叠后的成本怎样包含内部推导成本，共享前置是否重复计费。
...
如果还声称封装不影响路径优化，就要进一步证明或定义封装前后的成本如何对应。只有满足这些语义保持条件，边界关系才适合自动投影。否则，封装只能作为一种方便阅读的视图，不能直接参与严谨的路线计算。
