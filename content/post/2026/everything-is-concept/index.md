...
## 2026.8.19 修正：推导载荷属于 AND 组，而不是单条边

上文把问题引入、推导解答和理解材料都放在推导边上。在把这套模型整理成最终版本时，我发现这个归属在 AND 方案下并不成立。它不改变模型的主体结构，但改变了“推导的最小单元究竟是什么”，所以这里补一处修正。

问题出在 AND 方案。如果推出 $v$ 需要 $A$ 与 $B$ 共同成立，那么单独一条 $A \to v$ 并不足以证成 $v$；“旧概念遇到了什么局限、为什么需要 $v$”只有对整组前提才有意义。把它写在某一条边上，另一条边就会显得没有动机，或者被迫复述同一段话。

更严重的是边与方案是多对多的。若载荷挂在边上，一方面同一方案内各边必须给出同一个问题引入，另一方面同一条边又可能属于两套动机完全不同的方案，于是这两套方案也被迫共享同一个问题引入。把这两条约束的传递闭包展开，结论是：**只要两套方案共享一条边，它们就必须共用同一个“为什么”。**这显然不是知识的实际形态。

因此正确的归属是方案本身。问题引入、推导解答和理解材料都是定义在推导方案上的函数，而不是定义在边上的函数：

```text
Concept
├── definition          # 这个概念最终应当能够被重新推导出的定义
├── children[]          # 内部推导图引用的直接子概念
└── derivation_plans[]  # 以本图内节点为目标的推导方案

DerivationPlan          # AND 组：推导的最小单元，唯一的内容载体
├── target              # 本方案要推出的目标概念
├── premises[]          # 必须全部完成的前提概念（AND）
├── question            # 旧概念遇到什么局限，为什么需要 target
├── answer              # 由这组前提共同推出 target 的完整论证
└── materials[]         # 完成这一步推导所需的材料

Derivation              # 投影视图，由方案导出，不单独存储教学内容
├── source
├── target
└── plan_ref
```

这样一来，模型正好变成一张 AND/OR 超图：一套方案就是一条超边，前提集合是尾、目标是头；推导边退化为超边在有向图上的投影，只用来判断无环、给出拓扑序和可达性。回过头看，我在 v3 得到的那个结论——“Prompt 就是有向边”——只在每套方案都只有一个前提时才成立。

这处修正是保守的。单前提方案下方案与边一一对应，所以上文关于有效推导子图、无环性、拓扑序、递归展开约束和子图封装条件的结论全部不变，信号与系统案例中的 19 条边、5 条边、$\Delta C_0 = -14$ 和 $26\%$ 也不变。

唯一的实质变化是成本口径：结构成本应当数的是需要完成的推导方案数，也就是推导步数，而不是边数。

$$
C_0(R) = |\operatorname{sel}(R)|, \qquad
C_u^*(S,t) = \min_{R \in \mathcal{R}(S,t)} \sum_{P \in \operatorname{sel}(R)} w_u(P)
$$

其中 $\operatorname{sel}(R)$ 是这张有效推导子图所选中的方案集合，$w_u(P)$ 是完成整套方案的成本；$|E_R|$ 退居为“前提总数”这个辅助指标。当所有方案都只有一个前提时，$|\operatorname{sel}(R)| = |E_R|$，两种口径重合。另外还有一个附带好处：$R$ 本身就是一组方案，不必再从边集合反推当初选了哪套方案。

这也顺带回答了上一节遗留问题中的一半：解释一组 AND 条件如何共同起作用的材料，确实属于整套方案，而不属于其中某一条边。剩下没有解决的是，方案成本 $w_u(P)$ 应该用什么可观测量来度量，以及封装前后方案成本如何对应。

最后，这处修正本身也是一条边：v5 暴露出“载荷归属与 AND 语义冲突”的局限，由此推导出 v5.1。按照本文的说法，它同样是一次“问题引入 + 解答”，并被记录进了那张仍在生长的认知 DAG。
